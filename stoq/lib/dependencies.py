...
PYBARCODE_REQUIRED = (0, 7)
XMLTODICT_REQUIRED = (0, 8, 3)
ZOPE_INTERFACE_REQUIRED = (3, 0)
SUDS_REQUIRED = (0, 4)
NETIFACES_REQUIRED = (0, 8)
...
class DependencyChecker(object):
    ...
    def check(self):
        ...
        self._check_pybarcode(PYBARCODE_REQUIRED)
        self._check_xmltodict(XMLTODICT_REQUIRED)
        ...
        if platform.system() != 'Windows':
            ...
            self._check_pyserial(PYSERIAL_REQUIRED)         # Windows doesnt use pyserial. DLL only

        # License
        self._check_suds(SUDS_REQUIRED)
        self._check_netifaces(NETIFACES_REQUIRED)
...
You can find an older version of %s on it's homepage at\n%s""") % (
    ...
    def _check_suds(self, version):
        try:
            import suds
        except ImportError:
            self._missing(
                project="Suds",
                url='http://sourceforge.net/projects/python-suds/',
                version=version
            )

    def _check_netifaces(self, version):
        try:
            import netifaces
        except ImportError:
            self._missing(
    ...
    def _check_pybarcode(self, version):
        try:
            import barcode
        except ImportError:
            self._missing(project='PyBarcode',
                          url='https://pypi.python.org/pypi/pyBarcode/',
                          version=version)

    def _check_xmltodict(self, version):
        try:
            import xmltodict
        except ImportError:
            self._missing(project='xmltodict',
