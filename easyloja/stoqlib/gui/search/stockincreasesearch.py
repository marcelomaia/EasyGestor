...
##
##
""" Search dialogs for stock increases"""
...
from stoqlib.domain.stockincrease import StockIncrease
from stoqlib.gui.base.dialogs import run_dialog, get_current_toplevel
from stoqlib.gui.base.search import SearchDialog
from stoqlib.gui.dialogs.stockincreasedialog import StockIncreaseDetailsDialog
...
class StockIncreaseSearch(SearchDialog):
    title = _(u"Manual Stock Increase Search")
    size = (750, 500)
    search_table = StockIncrease
    selection_mode = gtk.SELECTION_MULTIPLE
    searchbar_result_strings = _(u"manual stock increase"), _(u"manual stock increases")
    search_by_date = True
    ...
    def _show_details(self, item):
        run_dialog(StockIncreaseDetailsDialog, get_current_toplevel(), self.conn,
                   item)
    ...
    def get_columns(self):
        return [Column('id', _('#'), data_type=int, width=50),
                ...
                Column('added_by.person.name', _('Added By'),
                       data_type=unicode, width=120),
                Column('total_items_added',
                       _('Items added'), data_type=Decimal, width=110),
    ...
    def on_print_button_clicked(self, button):
        orders = self.results.get_selected_rows()
        if len(orders) == 1:
            print_report(StockIncreaseReceipt, orders[0])
