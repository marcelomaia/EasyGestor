...
class PurchasedItemsSearch(SearchEditor):
    ...
    def get_columns(self):
        return [SearchColumn('purchase_id', title=_('Compra #'), data_type=int,
                             sorted=True),
                SearchColumn('status', title=_('Status'), data_type=str,
                             format_func=PurchaseOrder.translate_status),
                SearchColumn('description', title=_('Description'), data_type=str,
                             expand=True),
                SearchColumn('purchased', title=_('Purchased'), data_type=Decimal,
                             width=100),
                SearchColumn('invoice_number', title=_('Invoice number'), data_type=int,
                             width=100),
...
class _PurchasedItemsSearch(PurchasedItemsSearch):

    def __init__(self, conn):
        super(_PurchasedItemsSearch, self).__init__(conn)
