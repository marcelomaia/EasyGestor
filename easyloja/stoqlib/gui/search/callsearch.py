...
from stoqlib.domain.person import CallsView, ClientCallsView, Calls
...
class CallsSearch(SearchEditor):
    ...
    def setup_widgets(self):
        ...
        self.search.set_summary_label('price', label=_(u'Total:'),
                                      format='<b>%s</b>')
    ...
    def create_filters(self):
        ...
        if self._date:
            ...
            self.search.refresh()
        self.status_filter = ComboSearchFilter(_('Show sales with status'),
                                          self._get_status_values())
        self.search.add_filter(self.status_filter)

    def _get_status_values(self):
        items = [(Calls.statuses[s], s) for s in Calls.statuses]
        return items
    ...
    def get_columns(self):
        columns = [Column('date', title=_('Date'),
                ...
                SearchColumn('service', title=_('Service'),
                             data_type=str, expand=True),
                SearchColumn('price', title=_('Price'),
                             data_type=currency, width=60, expand=True),]
        if not self.person:
            columns.insert(1,
    ...
    def run_editor(self, obj):
        if self._reuse_transaction:
            ...
            retval = run_dialog(self.editor_class, get_current_toplevel(), self.conn,
            ...
            retval = run_dialog(self.editor_class, get_current_toplevel(), trans,
