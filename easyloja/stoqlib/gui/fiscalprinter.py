...
from stoqlib.gui.editors.saleeditor import SaleConfirmDateDialog
from stoqlib.gui.editors.tilleditor import TillOpeningEditor, TillClosingEditor, TillVerifyEditor
...
class FiscalPrinterHelper(gobject.GObject):
    ...
    def time_to_Z(self):
        timeZ = api.sysparam(self.conn).TIME_TO_EMIT_Z.split(':')
        nowTime = time.strftime('%H:%M').split(':')

        if int(nowTime[0]) > int(timeZ[0]):
            return True
        if int(nowTime[0]) < int(timeZ[0]):
            return False
        elif int(nowTime[1]) > int(timeZ[1]):
            return True
