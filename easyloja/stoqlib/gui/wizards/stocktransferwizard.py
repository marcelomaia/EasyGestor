...
class StockTransfer(BaseEditor):
    gladefile = 'StockTransfer'
    model_type = Settable
    title = u'Transferência de estoque'
    proxy_widgets = (
        'open_date',
        'receival_date',
        'destination_responsible',
    ...
    def __init__(self, conn):
        super(StockTransfer, self).__init__(conn)

    def setup_proxies(self):
        self._setup_widgets()
        self.proxy = self.add_proxy(self.model,
                                    StockTransfer.proxy_widgets)

    def create_model(self, trans):
        return Settable(open_date=datetime.date.today(),
                        receival_date=datetime.date.today(),
                        destination_branch=None,
                        destination_responsible=None,
                        source_responsible=None,
                        quantity=Decimal(1),
                        source_branch=get_current_branch(trans),
                        connection=trans)

    def _setup_widgets(self):
        curr_branch = get_current_branch(self.conn)
        branches = [(b.person.name, b)
                    for b in Person.iselect(IBranch, connection=self.conn)]
        branches.remove((curr_branch.person.name, curr_branch))
        self.destination_branch.prefill(branches)
        self.source_branch.set_text(curr_branch.person.name)
        employees = [(e.person.name, e)
                     for e in Person.iselect(IEmployee,
                                             connection=self.conn)]
        self.source_responsible.prefill(employees)
