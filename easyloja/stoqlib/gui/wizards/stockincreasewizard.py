...
from stoqlib.domain.stockincrease import StockIncrease, StockIncreaseItem
...
from stoqlib.gui.editors.increaseeditor import IncreaseItemEditor
...
from stoqlib.reporting.stockincreasereceipt import StockIncreaseReceipt
...
class StartStockIncreaseStep(WizardEditorStep):
    gladefile = 'StartStockIncreaseStep'
    model_type = StockIncrease
    ...
    def _fill_employee_combo(self):
        ...
        self.added_by.prefill(sorted(employees))
    ...
    def post_init(self):
        ...
        self.table1.set_focus_chain([self.confirm_date, self.branch,
                                     self.added_by, self.reason, self.cfop])
        self.register_validate_function(self.wizard.refresh_next)
        self.force_validation()

    def next_step(self):
        return IncreaseItemStep(self.wizard, self, self.conn, self.model)
...
class IncreaseItemStep(SellableItemStep):
    ...
    def get_sellable_view_query(self):
        ...
        return AND(branch_query,
                   Sellable.get_available_sellables_for_increase_query(self.conn))
    ...
    def validate(self, value):
        ...
        can_increase = self.model.get_items().count()
        self.wizard.refresh_next(can_increase)
    ...
    def post_init(self):
        ...
        self.slave.set_editor(IncreaseItemEditor)
...
class StockIncreaseWizard(BaseWizard):
    size = (775, 400)
    title = _('Manual Stock Increase')

    def __init__(self, conn):
        model = self._create_model(conn)

        first_step = StartStockIncreaseStep(conn, self, model)
    ...
    def _receipt_dialog(self):
        ...
        if yesno(msg, gtk.RESPONSE_YES,
            ...
            print_report(StockIncreaseReceipt, self.model)
        return
