...
from stoqlib.lib.validators import validate_email, validate_phone_number, validate_mobile_number
...
class _PersonEditorTemplate(BaseEditorSlave):
    ...
    def setup_proxies(self):
        ...
        self.mobile_number.set_mask('(00)00000-0000')
        self._setup_widgets()
    ...
    def on_phone_number__validate(self, widget, value):
        if value == '':
            return
        if not validate_phone_number(value):
            return ValidationError(_('%s is not a valid phone') % value)

    def on_mobile_number__validate(self, widget, value):
        if value == '':
            return
    ...
    def _setup_widgets(self):
        ...
        if facet_individual and facet_company:
            ...
            self.name_lbl.set_text('Nome/razão social:')
            ...
            self.company_frame.show()
            self.name_lbl.set_text('Razão social:')
