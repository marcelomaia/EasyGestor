...
class RenegotiationDetailsDialog(BaseEditor):
    ...
    def _setup_widgets(self):
        # if self.model.supplier:
        #     self.label5.set_text(_("Supplier:"))
        self._setup_columns()
        ...
        changes = PaymentChangeHistoryView.select_by_group(self.model.group, connection=self.conn)
    ...
    def on_details_button__clicked(self, button):
        if self.model.client:
            run_dialog(ClientDetailsDialog, get_current_toplevel(), self.conn, self.model.client)
        elif self.model.supplier:
            run_dialog(SupplierDetailsDialog, get_current_toplevel(), self.conn, self.model.supplier)
