...
class ClientDetailsDialog(BaseEditor):
    ...
    def _get_product_columns(self):
        return [Column("code", title=_("Code"), data_type=str,
                ...
                Column("salesperson_name", title=_("Salesperson"), data_type=str,
                       searchable=True),
    ...
    def _on_export_csv_button__clicked(self, widget):
        page = self.notebook2.get_current_page()                # get current tab
        sales, products, services, payments, calls = range(5)
        page_to_list = {sales: self.sales_list,
        ...
        run_dialog(CSVExporterDialog, get_current_toplevel(), self.conn, self.model, page_to_list.get(page, 0))
