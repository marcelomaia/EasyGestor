...
class BatchInitialStock(BatchEditor):
    title = _(u"Product  - Initial Stock")
    ...
    def __init__(self, conn, branch=None):
        ...
        if branch is None:
            ...
            self._branch = branch
        self.header_text = _(u"Registering initial stock for products in %s" %
                                 self._branch.person.name)
        BatchEditor.__init__(self, conn)
    ...
    def get_input_value(self, message, parent=None, min=0, max=9999):
        d = gtk.MessageDialog(parent,
                                ...
                                message)
        adj = gtk.Adjustment(value=0, lower=min, upper=max, step_incr=1, page_incr=10)
        entry = gtk.SpinButton(adj, climb_rate=0.0)
        entry.set_digits(0)
        entry.show()
    ...
    def _bacth_operation(self, selections):
        initial_stock = self.get_input_value('Valor do Estoque Inicial')
        for storable in selections:
            storable.initial_stock = initial_stock
            self.update_instance(storable)
