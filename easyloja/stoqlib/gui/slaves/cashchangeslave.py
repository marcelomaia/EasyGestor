...
class CashChangeSlave(BaseEditorSlave):
    ...
    gsignal('change_edited', object)
    gsignal('remaining_value', object)
    gsignal('paid_value_edited', object)
    ...
    #     # currency class doesn't ensure two decimal to low values like 1.99
    #     TWOPLACES = currency(10) ** -2
    #     sale_amount = self._get_total_amount()
    #     if value.quantize(TWOPLACES) < sale_amount.quantize(TWOPLACES):     # http://ebiinfo.com.br/ebimine/issues/314
    ...
    def on_received_value__content_changed(self, widget):
        ...
        if value is ValueUnset:
            value = '0.0'
        self.emit('paid_value_edited', currency(value))

        sale_amount = self._get_total_amount()
        change_value = currency(value) - sale_amount
        if change_value >= 0:
            self.kiwilabel2.set_text('Troco:')
            self.emit('change_edited', change_value)
            self.change_value_lbl.set_text(get_formatted_price(change_value))
            self.change_value_lbl.set_color('black')
        else:
            self.kiwilabel2.set_text('Falta:')
            self.emit('change_edited', 0)
            self.emit('remaining_value', change_value*-1)
            self.change_value_lbl.set_text(get_formatted_price(change_value*-1))
            self.change_value_lbl.set_color('red')
