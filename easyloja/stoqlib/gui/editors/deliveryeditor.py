...
from gtk._gtk import SELECTION_BROWSE
...
class DeliveryEditor(BaseEditor):
    ...
    def _setup_widgets(self):
        ...
        self.phone_number.set_tooltip_text(u'Número de telefone')
    ...
    def _show_address_editor(self):
        address = run_dialog(AddressSelectionDialog, get_current_toplevel(),
    ...
    def _search_client(self):
        phone = self.phone_number.get_text()
        if not phone:
            retval = run_dialog(ClientSearch, self, self.conn,
            ...
            phone = '%%{}%%'.format(phone)
            retval = Person.select(OR(LIKE(Person.q.phone_number, phone), LIKE(Person.q.mobile_number, phone)))
            persons = [p for p in retval]
            if persons:
                client = IClient(persons[0])
                self.client.select(client)
    ...
    def on_additional_info_button__clicked(self, button):
        if run_dialog(NoteEditor, get_current_toplevel(), self.conn, self.model, 'notes',
    ...
    def on_search_client__clicked(self, button):
        self._search_client()
