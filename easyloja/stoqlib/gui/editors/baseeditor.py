...
from kiwi.ui.listdialog import ListContainer, ListSlave
...
from stoqlib.gui.base.dialogs import BasicDialog, BasicWrappingDialog, run_dialog, get_current_toplevel
...
class BaseEditor(BaseEditorSlave):
    ...
    def _add_help_button(self, section):
        ...
        button.show()
        change_button_appearance(button, gtk.STOCK_HELP, 'Ajuda')
...
class BaseRelationshipEditorSlave(GladeSlaveDelegate):
    ...
    def _setup_widgets(self):
        ...
        change_button_appearance(self.add_button, gtk.STOCK_ADD, 'Adicionar')
        change_button_appearance(self.relations_list.remove_button,
                                 gtk.STOCK_REMOVE, 'Remover')
        change_button_appearance(self.relations_list.edit_button,
                                 gtk.STOCK_EDIT, 'Editar')
    ...
    def add(self):
        ...
        res = run_dialog(self.editor, get_current_toplevel(), self.conn, model)
    ...
    def edit(self, model):
        return run_dialog(self.editor, get_current_toplevel(), self.conn, model)
...
class BatchEditor(BasicDialog):
    ...
    def setup_widgets(self):
        ...
        self.klist = self._get_objects()
        self.object_list = ObjectList(self._get_columns(), self.klist,
                                gtk.SELECTION_MULTIPLE)
        self.object_list.connect('selection-changed', self.on_object_list_selection_changed)
        self.main.remove(self.main.get_child())
        self.main.add(self.object_list)
        self.object_list.show()

    def setup_action_buttons(self):
        action_area = self.action_area
        self.batch_button = gtk.Button(_('Editar Lote'))
        self.batch_button.connect("clicked", self.on_batch_button_clicked)
        action_area.pack_start(self.batch_button)
        self.batch_button.set_sensitive(False)
        self.batch_button.show()

    def update_instance(self, instance):
        self.object_list.update(instance)

    def on_batch_button_clicked(self, *args, **kwargs):
        selection = self.object_list.get_selected_rows()
        self._bacth_operation(selection)
        self.object_list.refresh()
        self.batch_button.set_sensitive(False)

    def on_object_list_selection_changed(self, *args, **kwargs):
        if self.object_list.get_selected_rows():
            self.batch_button.set_sensitive(True)
        else:
            self.batch_button.set_sensitive(False)
    ...
    def _bacth_operation(self, rows):
        raise NotImplementedError
