...
from stoqlib.gui.slaves.taxslave import SaleItemICMSSlave, SaleItemIPISlave, SaleItemPISSlave, SaleItemCOFINSSlave
...
class SaleQuoteItemEditor(BaseEditor):
    ...
    def _setup_taxes(self):
        ...
        self.pis_slave = SaleItemPISSlave(self.conn, self.model.pis_info)
        self.add_tab(_('PIS'), self.pis_slave)

        self.cofins_slave = SaleItemCOFINSSlave(self.conn, self.model.cofins_info)
...
class SaleConfirmDateDialog(BaseEditor):
    gladefile = "SaleConfirmDateDialog"
    title = "Ajuste da Venda"
    model_type = Sale
    proxy_widgets = ("confirm_date",)
    size = (300, 150)

    def __init__(self, conn, model, visual_mode=False):
        BaseEditor.__init__(self, model=model, conn=conn)
        self._setup_widgets()
    ...
    def on_confirm(self):
        warning('Os pagamentos desta venda serão confirmados no caixa atualmente aberto.')
