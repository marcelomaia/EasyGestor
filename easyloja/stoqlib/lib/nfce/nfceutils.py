...
from stoqlib.domain.nfe import NFeCityData
...
def get_cities_by_name(city, limit=20):
    ...
    results = NFeCityData.select(AND(ILIKE(NFeCityData.q.city_name, city_name)),
...
def strip_accents(string):
    ...
    string = unicodedata.normalize('NFKD', string).replace('/', '-')
...
def main(args):
    ...
    # a = get_city_code('ANANINDEUA')
    # sale = Sale.selectOneBy(id=17615, connection=conn)
    sale = Sale.selectOneBy(id=17614, connection=conn)

    run_dialog(BranchDialog,  None, conn, sale)
    conn.commit()
