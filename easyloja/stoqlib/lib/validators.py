...
def validate_phone_number(phone_number):
    ...
    return digits in range(10, 11)


def validate_mobile_number(phone_number):
    phone_number = raw_phone_number(phone_number)
    digits = len(phone_number)
    return digits in range(11, 12)
...
def validate_cpf(cpf):
    ...
    seq = range(10)
    seq_invalid_cpf = [str(num) * 11 for num in seq]
    if cpf in seq_invalid_cpf:
        return False
...
def validate_cnpj(cnpj):
    ...
    seq = range(10)
    seq_invalid_cnpj = [str(num) * 14 for num in seq]  # invalidate all 000... and 111... etc...
    if cnpj in seq_invalid_cnpj:
        return False
...
def validate_sellable_description(value):
    if value.strip() != value:
        return False
