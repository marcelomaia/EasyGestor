...
class PurchaseOrderReport(BaseStoqReport):
    ...
    def _get_items_table_columns(self, include_received):
        ...
        cols = [
            ...
            OTC(_("IPI"), lambda obj: get_formatted_cost(obj.ipi_cost),
                width=70, align=RIGHT),
    ...
    def _add_items_table(self, items):
        ...
        for item in items:
            ...
            total_value += item.quantity * (item.cost + item.ipi_cost)
            total_received += item.quantity_received * (item.cost + item.ipi_cost)
