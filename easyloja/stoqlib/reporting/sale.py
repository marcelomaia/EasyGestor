...
class SaleOrderReport(BaseStoqReport):
    ...
    def _identify_client(self):
        ...
        a = "%s, n%s - %s\n%s" % (addr._get_street(),
                                  addr._get_streetnumber(),
                                  addr._get_district(),
                                  addr._get_complement())
        ...
        cols = [TC('', style='Normal-Bold', width=80),
                TC('', expand=True, truncate=False),
                TC('', style='Normal-Bold', width=130), TC('', expand=True)]

        doc_label = 'CNPJ' if company else 'CPF'

        data = [[_(u'Client:'), client, _(u'Phone number:'), phone_number],
                [doc_label, document, _(u'Mobile number'), mobile_number],
                [u'', u'', _(u'E-mail:'), email],                            # To fit in second column
                [_(u'Address:'), a, _(u'City/State:'), address[1]]]

        if company is not None:
            data.append(['IE:', company.get_state_registry_number(),
                         'Nome fantasia:', company.fancy_name])
    ...
    def _setup_items_table(self):
        ...
        data = [['', _(u'Discount:'), discount],
                ['', _(u'Total:'), total]]

        card_total_fee = self.sale.get_card_total_fee()
        if card_total_fee:
            data.insert(0, ['', _(u'Total Tx. Cartao:'), get_formatted_price(card_total_fee)])
    ...
    def get_title(self):
        ...
        if self.sale.expire_date:
            extra += u';'
            extra += u' Data de expiração: %s' % self.sale.expire_date.strftime('%x')
