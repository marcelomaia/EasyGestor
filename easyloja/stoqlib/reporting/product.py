...
class SimpleProductReport(ObjectListReport):
    ...
    def _setup_items_table(self):
        ...
        total_expected = sum([p.total_expected for p in self._products], Decimal(0))
        self.add_summary_by_column(_(u'Quantity'), format_quantity(total))
        self.add_summary_by_column(_(u'Total esperado'), get_formatted_price(total_expected))
        self.add_object_table(self._products, self.get_columns(),
                              summary_row=self.get_summary_row())
...
class ProductsSoldReport(ObjectListReport):
    ...
    def _add_sale_notes(self):
        ...
        """
        d = {}
        for siv in self.objectlist:
            if not d.get(siv.category, None):
                d[siv.category] = siv.total_price
            else:
                d[siv.category] += siv.total_price
        ...
        for i in d:
            details_str += 'Categoria: {} Total: {} \n'.format(i, format_price(d[i]))
