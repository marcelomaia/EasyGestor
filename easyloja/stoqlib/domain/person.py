...
class EmployeeRole(Domain):
    ...
    def get_description(self):
        return (self.name and " (%s)" % self.name or "") + self.name
...
class Person(Domain):
    ...
    def get_default_category(self):
        return PersonCategoryPaymentInfo.selectOneBy(person=self.id,
                                                     is_default=True,
                                                     connection=self.get_connection())
...
class PersonAdaptToIndividual(PersonAdapter):
    ...
    birth_month = IntCol(default=None)
...
class PersonAdaptToCompany(PersonAdapter):
    ...
    responsible_name = UnicodeCol(default='')
...
class PersonAdaptToBranch(PersonAdapter):
    ...
    key_client = UnicodeCol(default='')
    aliquota_credito = PercentCol(default=0)
...
class PersonCategoryPaymentInfo(Domain):
    person = ForeignKey('Person')
    payment_category = ForeignKey('PaymentCategory')
    is_default = BoolCol(default=False)

    @classmethod
    def get_payments(cls, person, conn):
        person_category = cls.selectBy(person=person, connection=conn)
        person_category = sorted([p for p in person_category],
                                 key=attrgetter('is_default'),
                                 reverse=True)
        return [p.payment_category for p in person_category]
...
class ClientView(Viewable):
    ...
    columns = dict(
        ...
        birth_date=PersonAdaptToIndividual.q.birth_date,
        birth_month=PersonAdaptToIndividual.q.birth_month,
    ...
    def get_description(self):
        # Get fancy_name
        if self.fancy_name == None:
            fancy_name = ''
        else:
            fancy_name = self.fancy_name and ' (%s) ' % self.fancy_name

        # Get rg_number
        if self.rg_number == None:
            rg_number = ''
        else:
            rg_number = self.rg_number and ' %s ' % self.rg_number

        # Return the correct information
        if fancy_name != '':
            return str(fancy_name) + self.name
        elif rg_number != '':
            return str(rg_number)[-4:] + self.name
        else:
            return self.name
...
class CallsView(Viewable):
    ...
    columns = dict(
        ...
        price=Calls.q.price,
        service=Sellable.q.description,
    )
...
class SalesPersonView(Viewable):
    columns = dict(id=PersonAdaptToSalesPerson.q.id,
                   ...
                   phone_number=Person.q.phone_number,
                   mobile_number=Person.q.mobile_number)

    joins = [INNERJOINOn(None, Person,
                         PersonAdaptToSalesPerson.q.originalID == Person.q.id)]

    @property
    def sales_person(self):
        return PersonAdaptToSalesPerson.get(self.id,
                                            connection=self.get_connection())
