...
from stoqlib.database.orm import PriceCol, DecimalCol, QuantityCol, Field, Viewable
...
class Product(Domain):
    ...
    @ivar pis_template: PIS tax template, brazil specific
    @ivar cofins_template: COFINS tax template, brazil specific
    """
    ...
    product_types = {PRODUCT: u'Produto',
                     VEHICLE: u'Veículo',
                     DRUG: u'Medicamento',
                     WEAPONRY: u'Armamento',
                     FUEL: u'Combustível'}
    ...
    pis_template = ForeignKey('ProductPisTemplate', default=None)
    cofins_template = ForeignKey('ProductCofinsTemplate', default=None)
    ...
    product_type = IntCol(default=PRODUCT)
...
class ProductHistory(Domain):
    ...
    quantity_increased = QuantityCol(default=None)
    ...
    increased_date = DateTimeCol(default=None)
    ...
    @classmethod
    def add_increased_item(cls, conn, branch, item):
        """
        Adds a increased item, populates the ProductHistory table informing
        how many items wore manually decreased from stock.

        @param conn: a database connection
        @param branch: the source branch
        @param item: a StockIncreaseItem instance
        """
        cls(branch=branch, sellable=item.sellable,
            quantity_increased=item.quantity,
            increased_date=datetime.date.today(), connection=conn)
...
class ProductStockItem(Domain):
    ...
    quote_quantity = QuantityCol(default=0)
...
class ProductAdaptToStorable(ModelAdapter):
    ...
    def _check_rejected_stocks(self, stocks, quantity, check_logic=False):
        for stock_item in stocks:
            ...
            if base_qty < quantity and not sysparam(self.get_connection()).NEGATIVE_STOCK:
                raise StockError(
    ...
    def _has_qty_available(self, quantity, branch):
        ...
        if sysparam(self.get_connection()).NEGATIVE_STOCK:
            qty_ok = True
        else:
    ...
    def decrease_stock(self, quantity, branch):
        ...
        old_quantity = stock_item.quantity
        stock_item.quantity -= quantity
        if stock_item.quantity < 0 and not sysparam(self.get_connection()).NEGATIVE_STOCK:
            raise ValueError(_("Quantity cannot be negative"))

        if not sysparam(self.get_connection()).NEGATIVE_STOCK:
...
class Attribute(Domain):
    """ A attribute, creates attribute inside of a GroupAttribute
    """
    description = UnicodeCol(default='')
    value = UnicodeCol(default='')
    notes = UnicodeCol(default='')
    group_attribute = ForeignKey('GroupAttribute', default=None)


# TODO move these to another place!
class GroupAttribute(Domain):
    """ A attribute group, creates a set of attribute
    """
    (TYPE_TEXT,
     TYPE_COLOR,
     TYPE_BOOLEAN) = range(3)

    type = IntCol(default=TYPE_TEXT)
    description = UnicodeCol(default='')
    notes = UnicodeCol(default='')

    def get_attributes(self):
        conn = self.get_connection()
        return Attribute.selectBy(group_attribute=self,
                                  connection=conn)


class ProductAttribute(Domain):
    """ A middle class between a Product and a Attribute
    """
...
class _ProductAttribute():
    """ Dummy class to help ProductAttribute
    """
    price = None
    quantity = None
    description = None
...
class ProductSerialNumber(Domain):
    """A product and it's related component eg other product
    """
    (IN_STOCK,
     SOLD,
    ...
    branch = ForeignKey('PersonAdaptToBranch', default=None)
    status = IntCol(default=IN_STOCK)
    serial_number = UnicodeCol(default=u'')
