...
##
from sqlobject.sqlbuilder import RIGHTJOINOn
from sqlobject.viewable import ViewableSelectResults
from stoqlib.database.orm import Viewable, Field, Alias
from stoqlib.database.orm import const, AND, INNERJOINOn, LEFTJOINOn, OR, ISNULL, ISNOTNULL
...
from stoqlib.domain.person import (Person, PersonAdaptToSupplier,
                                   ...
                                   PersonAdaptToClient, PersonAdaptToEmployee, PersonAdaptToSalesPerson)
...
class ProductFullStockView(Viewable):
    ...
    ICMSAlias = Alias(ProductTaxTemplate, 'icms_alias')
    IPIAlias = Alias(ProductTaxTemplate, 'ipi_alias')
    PISALias = Alias(ProductTaxTemplate, 'pis_alias')
    COFINSAlias = Alias(ProductTaxTemplate, 'cofins_alias')
    ...
    columns = dict(
        ...
        total_expected=Sellable.q.base_price * const.COALESCE(const.SUM(ProductStockItem.q.quantity +
                                                                        ProductStockItem.q.logic_quantity), 0),
        total_cost=Sellable.q.cost * const.COALESCE(const.SUM(ProductStockItem.q.quantity +
                                                              ProductStockItem.q.logic_quantity), 0),
        stock_quote=ProductStockItem.q.quote_quantity,
        ...
        cofins_name=COFINSAlias.q.name,
    ...
    joins = [
        ...
        LEFTJOINOn(None, CfopData,
                   CfopData.q.id ==
                   Sellable.q.default_sale_cfopID),
        # icms, ipi, pis, cofins
        LEFTJOINOn(None, ProductIcmsTemplate,
                   ProductIcmsTemplate.q.id ==
                   Product.q.icms_templateID),
        LEFTJOINOn(None, ProductIpiTemplate,
                   ProductIpiTemplate.q.id ==
                   Product.q.ipi_templateID),
        LEFTJOINOn(None, ProductPisTemplate,
                   ProductPisTemplate.q.id ==
                   Product.q.pis_template),
        LEFTJOINOn(None, ProductCofinsTemplate,
                   ProductCofinsTemplate.q.id ==
                   Product.q.cofins_template),
        LEFTJOINOn(None, ICMSAlias,
                   ICMSAlias.q.id ==
                   ProductIcmsTemplate.q.product_tax_templateID),
        LEFTJOINOn(None, IPIAlias,
                   IPIAlias.q.id ==
                   ProductIpiTemplate.q.product_tax_templateID),
        LEFTJOINOn(None, PISALias,
                   PISALias.q.id ==
                   ProductPisTemplate.q.product_tax_templateID),
        LEFTJOINOn(None, COFINSAlias,
                   COFINSAlias.q.id ==
                   ProductCofinsTemplate.q.product_tax_templateID),
...
class CustomDayStockView(ProductFullStockView):
    columns = ProductFullStockView.columns.copy()
    columns.update(dict(
        stock_custom_day=const.COALESCE(const.SUM(ProductStockItem.q.quantity +
                                                  ProductStockItem.q.logic_quantity), 0.0)
                         - const.COALESCE(const.SUM(ProductHistory.q.quantity_received), 0.0)
                         - const.COALESCE(const.SUM(ProductHistory.q.quantity_increased), 0.0)
                         + const.COALESCE(const.SUM(ProductHistory.q.quantity_sold), 0.0)
                         + const.COALESCE(const.SUM(ProductHistory.q.quantity_decreased), 0.0),
    ))
    joins = ProductFullStockView.joins[:]
    joins.append(LEFTJOINOn(None, ProductHistory,
                            ProductHistory.q.sellableID == Sellable.q.id), )
...
class SoldServiceView(Viewable):
    ...
    columns = dict(
        ...
        total_price=const.SUM(SaleItem.q.quantity * SaleItem.q.price),
    )

    joins = [
        LEFTJOINOn(None, SaleItem,
                   Sellable.q.id == SaleItem.q.sellableID),
        RIGHTJOINOn(None, Service,
    ...
    @classmethod
    def select_by_branch_date(cls, query, date,
...
class SoldItemView(Viewable):
    ...
    columns = dict(
        ...
        total_cost=const.SUM(SaleItem.q.quantity * SaleItem.q.average_cost),
        total_price=const.SUM(SaleItem.q.quantity * SaleItem.q.price),
        client_category=SaleItem.q.category_name,
    )

    joins = [
        LEFTJOINOn(None, SaleItem,
                   Sellable.q.id == SaleItem.q.sellableID),
        # RIGHTJOINOn(None, Product,
...
class ProductRevenueView(Viewable):
    """Stores information about all sale items, including the average cost
    of the sold items.
    """
    columns = dict(
    ...
    joins = [
        ...
        LEFTJOINOn(None, SaleItemIcms,
                   SaleItem.q.icms_infoID == SaleItemIcms.q.id),
        LEFTJOINOn(None, SellableCategory,
                   SellableCategory.q.id == Sellable.q.categoryID),
    ]
    ...
    @classmethod
    def select_by_branch_date(cls, query, date, view_service=False,
                              having=None, connection=None):
        if date:
            if isinstance(date, tuple):
                date_query = AND(const.DATE(Sale.q.confirm_date) >= date[0],
        ...
        service_query = ISNULL(Product.q.sellableID) if view_service else ISNOTNULL(Product.q.sellableID)

        if query:
            query = AND(query, service_query)
        else:
            query = service_query
    ...
    @classmethod
    def select_by_branch_date(cls, query, branch, date, view_service,
                              having=None, connection=None):
        if branch:
            branch_query = Sale.q.branchID == branch.id
            if query:
...
class StockIncreaseItemsView(Viewable):
    """Stores information about all stock decrease items
    """
    columns = dict(
        id=StockIncreaseItem.q.id,
        date=StockIncrease.q.confirm_date,
        added_by_name=Person.q.name,
        quantity=StockIncreaseItem.q.quantity,
        sellable=StockIncreaseItem.q.sellableID,
        unit_description=SellableUnit.q.description,
    )

    joins = [
        INNERJOINOn(None, StockIncrease,
                    StockIncreaseItem.q.stock_increaseID == StockIncrease.q.id),
        LEFTJOINOn(None, Sellable,
                   StockIncreaseItem.q.sellableID == Sellable.q.id),
        LEFTJOINOn(None, SellableUnit,
                   Sellable.q.unitID == SellableUnit.q.id),
        INNERJOINOn(None, PersonAdaptToEmployee,
                    StockIncrease.q.added_byID == PersonAdaptToEmployee.q.id),
        INNERJOINOn(None, Person,
                    PersonAdaptToEmployee.q.originalID == Person.q.id),
    ]
...
class PurchasedItemAndStockView(Viewable):
    ...
    clause = AND(PurchaseOrder.q.branchID == ProductStockItem.q.branchID, )
...
class SaleItemsView(Viewable):
    ...
    columns = dict(
        ...
        total=SaleItem.q.price * SaleItem.q.quantity,
...
class ReceivingItemView(Viewable):
    ...
    columns = dict(
        ...
        unit_description=SellableUnit.q.description,
        supplier_name=Person.q.name,
        total=ReceivingOrderItem.q.quantity * ReceivingOrderItem.q.cost
    )
...
class SalesPerSalesPersonView(Viewable):
    columns = dict(id=Sellable.q.id,
                   code=Sellable.q.code,
                   description=Sellable.q.description,
                   branch=Sale.q.branchID,
                   average_price=const.AVG(SaleItem.q.price),
                   quantity=const.SUM(SaleItem.q.quantity),
                   total=const.COALESCE(const.SUM(SaleItem.q.quantity *
                                                  SaleItem.q.price), 0))

    joins = [INNERJOINOn(None, Sellable,
                         Sellable.q.id == SaleItem.q.sellableID),
             INNERJOINOn(None, Sale,
                         Sale.q.id == SaleItem.q.saleID),
