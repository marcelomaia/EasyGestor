...
class ReceivingOrderItem(Domain):
    ...
    def get_total(self):
        ...
        ipi = self.purchase_item.ipi_cost
        return currency(self.quantity * (cost + ipi))
...
class ReceivingOrder(Domain):
    ...
    (FREIGHT_FOB_PAYMENT,
     ...
     FREIGHT_CIF_UNKNOWN,
     FREIGHT_CIF_INVOICE,
     FREIGHT_FOB_FIRST_INSTALLMENT) = range(5)
    ...
    freight_types = {FREIGHT_FOB_PAYMENT: _(u"FOB - Freight value "
                     ...
                     FREIGHT_CIF_INVOICE: _(u"CIF - Freight value highlighted "
                                            "on invoice"),
                     FREIGHT_FOB_FIRST_INSTALLMENT: _(u'FOB - Freight value '
                                                      "on first installment")}

    FOB_FREIGHTS = (FREIGHT_FOB_PAYMENT,
                    FREIGHT_FOB_INSTALLMENTS,
                    FREIGHT_FOB_FIRST_INSTALLMENT,)
    ...
    freight_due_date = DateTimeCol(default=None)
    ...
    invoice_serie = IntCol(default=None)
    nfe_key = UnicodeCol(default='')
    ...
    def confirm(self):
        ...
        FiscalBookEntry.create_product_entry(
            ...
            self.icms_total, self.ipi_total, self.nfe_key)
        ...
        if difference != 0:
            ...
            if self.freight_type == self.FREIGHT_FOB_FIRST_INSTALLMENT:
                if payments_number > 0:
                    freight_on_fist_installment = self.freight_total
                    first_payment = payments[0]
                    new_value = first_payment.value + freight_on_fist_installment
                    first_payment.update_value(new_value)
            else:
                if payments_number > 0:
    ...
    def _create_freight_payment(self):
        ...
        out_payment = money_method.create_outpayment(
            ...
            due_date=self.freight_due_date,
