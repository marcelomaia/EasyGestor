...
from stoqlib.gui.dialogs.usefuldialogs import DatePicker
...
class Payment(Domain):
    ...
    def pay(self, paid_date=None, paid_value=None, account=None):
        ...
        conn = get_connection()
        if sysparam(conn).CHOSE_SALE_CONFIRM_DATE:
            model = Settable(date=datetime.date.today(), text='alterando data de: ' + self.description)
            data = run_dialog(DatePicker, self._parent, conn, model)
            if data:
                self.paid_date = data.date
    ...
    def get_card_fee_value(self):
        # FIXME este import fica aqui ate acharmos outra soluçao
        from stoqlib.domain.payment.method import CreditCardData
        credit_card = CreditCardData.selectOneBy(connection=self.get_connection(),
                                                 payment=self)
        return credit_card.fee_value
