...
class PurchaseItem(Domain):
    ...
    def get_total(self):
        return currency(self.quantity * (self.cost + self.ipi_cost))

    def get_total_sold(self):
        return currency(self.quantity_sold * self.cost)

    def get_received_total(self):
        return currency(self.quantity_received * (self.cost + self.ipi_cost))
...
class PurchaseOrder(Domain):
    ...
    statuses = {ORDER_CANCELLED: _(u'Cancelled'),
                ...
                ORDER_RENEGOTIATED: _(u'Renegotiated')}
    ...
    def set_renegotiated(self):
        ...
        self.status = PurchaseOrder.ORDER_RENEGOTIATED
...
class PurchaseItemView(Viewable):
    ...
    columns = dict(
        ...
        total=(PurchaseItem.q.cost+PurchaseItem.q.ipi_cost) * PurchaseItem.q.quantity,
        total_received=(PurchaseItem.q.cost+PurchaseItem.q.ipi_cost) * PurchaseItem.q.quantity_received,
        total_sold=(PurchaseItem.q.cost+PurchaseItem.q.ipi_cost) * PurchaseItem.q.quantity_sold,
...
class PurchaseOrderView(Viewable):
    ...
    columns = dict(
        ...
        nfe_key=ReceivingOrder.q.nfe_key,
        invoice_number=ReceivingOrder.q.invoice_number,
        invoice_serie=ReceivingOrder.q.invoice_serie,
        ...
        ipi_total=const.SUM(PurchaseItem.q.ipi_cost),
        subtotal=const.SUM((PurchaseItem.q.cost + PurchaseItem.q.ipi_cost) * PurchaseItem.q.quantity),
        total=const.SUM((PurchaseItem.q.cost + PurchaseItem.q.ipi_cost) * PurchaseItem.q.quantity) - \
