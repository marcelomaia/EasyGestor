...
class BasePIS(BaseTax):
    ...
    calculo = IntCol(default=CALC_PERCENTAGE)
...
class SaleItemPis(BasePIS):
    ...
    def update_values(self):
        from stoqlib.domain.sale import SaleItem
        sale_item = SaleItem.selectOneBy(pis_info=self,
                                         connection=self.get_connection())
        self.q_bc_prod = sale_item.quantity
        ...
        if self.cst == 99 and sale_item.sale.branch.crt in [1, 2]:
            self.v_bc = 0
            self.p_pis = 0
            self.v_pis = 0
            return
        cost = self._get_item_cost(sale_item)
        self.v_bc = sale_item.quantity * (sale_item.price - cost)
        if self.p_pis is not None:
            self.v_pis = self.v_bc * self.p_pis / 100

    @classmethod
    def get_tax_template(cls, sale_item):
        return sale_item.sellable.product.pis_template

    #
    # Private API
    #

    def _get_item_cost(self, sale_item):
        from stoqlib.domain.sale import SaleItem

        if isinstance(sale_item, SaleItem):
            return sale_item.average_cost

        return 0


class SaleItemCofins(BaseCOFINS):
    ...
    def update_values(self):
        from stoqlib.domain.sale import SaleItem
        sale_item = SaleItem.selectOneBy(cofins_info=self,
                                         connection=self.get_connection())
        self.q_bc_prod = sale_item.quantity
    ...
    @classmethod
    def get_tax_template(cls, sale_item):
        return sale_item.sellable.product.cofins_template
