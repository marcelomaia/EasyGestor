...
from stoqlib.database.orm import AND, const, OR
from stoqlib.database.orm import ForeignKey, UnicodeCol, DateTimeCol, IntCol, StringCol
...
from stoqlib.domain.interfaces import (IContainer,
                                       IPaymentTransaction,
                                       IDelivery, IStorable, IEmployee)
from stoqlib.domain.nfe import NFESaleHistory, NFCESaleHistory
...
class SaleItem(Domain):
    ...
    price = PriceCol()
    category_name = UnicodeCol(default='')
    ...
    def _create(self, id, **kw):
        if not 'kw' in kw:
            ...
            kw['pis_info'] = SaleItemPis(connection=conn)
            kw['cofins_info'] = SaleItemCofins(connection=conn)
        ...
        if self.sellable.product:
            ...
            self.pis_info.set_from_template(self.sellable.product.pis_template)
            self.cofins_info.set_from_template(self.sellable.product.cofins_template)
    ...
    def sell(self, branch):
        ...
        if storable:
            ...
            # recurrency = Recurrency.selectOneBy(sellable=self.sellable, connection=conn)
            # if recurrency:
            #     query = "select localtimestamp + interval '%s %s' " % (recurrency.period,
            #                                                            recurrency.type_period)
            #     shipping_date = conn.queryOne(query)[0]
            #     schedule = SaleItemSchedule(sale_item=self,
            #                                 shipping_date=shipping_date,
            #                                 is_sent=False,
            #                                 connection=conn)
            #     conn.add_created_object(schedule)
            #     conn.commit()
    ...
    def get_nfe_pis_info(self):
        return self.pis_info

    def get_nfe_cofins_info(self):
        return self.cofins_info
...
class SaleReturnedItem(Domain):
    ...
    @classmethod
    def can_returned(cls, sale_item, conn):
        return cls.selectOneBy(sale_item=sale_item, connection=conn) is None and sale_item.sale.status in (
            Sale.STATUS_CONFIRMED, Sale.STATUS_PAID)
...
class SaleReturned():
    ...
    def __init__(self, sale, conn):
        items = SaleReturnedItemsView.select(
            SaleReturnedItemsView.q.sale_id == sale,
            connection=conn)
        tl_returned = Decimal(0.0)
        for item in items:
            tl_returned += item.price * item.quantity
        self.total_returned = str(tl_returned)
...
class Sale(Domain):
    ...
    tab = ForeignKey('SaleTab', default=None)
    ...
    @classmethod
    def get_last_sale(cls, conn):
        """Fetch the last confirmed sale
        @param conn: a database connection
        """
        results = cls.select(OR(cls.q.status == cls.STATUS_CONFIRMED,
                                cls.q.status == cls.STATUS_PAID),
                             orderBy='-id',
                             connection=conn).limit(1)
        if results:
            return results[0]
    ...
    def get_last_invoice_number(cls, conn):
        ...
        """
        if not get_plugin_manager().is_active('nfe') and not get_plugin_manager().is_active('nfe2'):
            return 1

        query = Sale.q.branchID == get_current_branch(conn).id
        return cls.select(query, connection=conn).max('invoice_number') or 0

    @classmethod
    def get_last_daily_code(cls, conn):
        begin = datetime.datetime.now().replace(hour=0,
                                                minute=0,
                                                second=0)
        end = begin.replace(hour=23,
                            minute=59,
                            second=59)
    ...
    def get_last_invoice_number_from_branch(cls, conn, branch):
        ...
        if not get_plugin_manager().is_active('nfe') and not get_plugin_manager().is_active('nfe2'):
            return 1
        return Sale.selectBy(branch=branch, connection=conn).max('invoice_number') or 0
    ...
    def _decrease_composed_stock(self, item, conn):
        ...
        sd = StockDecrease(responsible=user,
                           removed_by=employee,
                           branch=branch,
                           status=StockDecrease.STATUS_INITIAL,
                           cfop=cfop,
                           reason=u'Fabricação própria: {}, '
        ...
        for item in self.get_items():
            ...
            if product:
                ...
                if product.is_composed:
                    self._decrease_composed_stock(item, conn)
            item.sell(branch)
    ...
    def get_total_products_sale_amount(self):
        """
        Fetches the total value  paid by the client, considering only products
        It can be calculated as::

            Sale total = Sum(product prices) + surcharge +
                             interest - discount
        ...
        surcharge_value = self.surcharge_value or Decimal(0)
        discount_value = self.discount_value or Decimal(0)
        subtotal = self.get_sale_products_subtotal()
        total_amount = subtotal + surcharge_value - discount_value
        return currency(total_amount)
    ...
    def get_sale_products_subtotal(self):
        """Fetch the subtotal for the sale, eg the sum of the
        prices for of all products
        @returns: subtotal
        """
        total = 0
    ...
    def get_sale_services_subtotal(self):
        """Fetch the subtotal for the sale, eg the sum of the
        prices for of all services
        @returns: subtotal
        """
        total = 0
    ...
    def get_card_total_fee(self):
        """
            @return: total card fee
        """
        total_card_fee = Decimal(0)
        for payment in self.group.payments:
            if payment.method.method_name == 'card':
                total_card_fee += payment.get_card_fee_value()
...
class SaleTab(Domain):
    """This class refers to a sale,
    """

    (STATUS_OPENED,
     STATUS_CLOSED) = range(2)

    tab_num = IntCol(default=None)
    table_num = IntCol(default=None)
    color = StringCol(default=None)
    sale = ForeignKey('Sale')
...
class SaleAdaptToPaymentTransaction(object):
    ...
    def _add_inpayments(self):
        ...
        if till is None:
            # error(u'Caixa não aberto ainda!')
            raise ValueError(
                _(u'Caixa não aberto ainda!'))
    ...
    def _revert_fiscal_entry(self, invoice_number):
        entry = FiscalBookEntry.selectBy(
...
class SaleView(Viewable):
    ...
    Person_User = Alias(PersonAdaptToUser, 'person_user')
    ...
    columns = dict(
        ...
        user_logged=Person_User.q.username,
        ...
        nfe_invoice=NFESaleHistory.q.number,
        nfce_invoice=NFCESaleHistory.q.number,
    )
    ...
    joins = [
        ...
        LEFTJOINOn(None, Person_User,
                   PersonAdaptToSalesPerson.q.originalID == Person_User.q.originalID),
        LEFTJOINOn(None, Person,
                   Person.q.id == Person_User.q.originalID),
        LEFTJOINOn(None, SaleItemIpi,
                   SaleItemIpi.q.id == SaleItem.q.ipi_infoID),
        LEFTJOINOn(None, PersonAdaptToIndividual,
                   PersonAdaptToIndividual.q.originalID == Person_Client.q.id),
        LEFTJOINOn(None, PersonAdaptToCompany,
                   PersonAdaptToCompany.q.originalID == Person_Client.q.id),
        ...
        # NFe
        LEFTJOINOn(None, NFESaleHistory,
                   NFESaleHistory.q.saleID == Sale.q.id),
        # NFCe
        LEFTJOINOn(None, NFCESaleHistory,
                   NFCESaleHistory.q.saleID == Sale.q.id),
    ]
...
class SaleTabView(SaleView):
    """ This class is almost a copy of SaleView, add queries to a tab.
    """

    Person_Client = Alias(Person, 'person_client')
    Person_SalesPerson = Alias(Person, 'person_sales_person')
    # 28/11/12 Leonam Silva
    Person_User = Alias(PersonAdaptToUser, 'person_user')
    ##########################
    columns = dict(
        id=Sale.q.id,
        ...
        total=const.SUM(SaleItem.q.price * SaleItem.q.quantity) - \
              Sale.q.discount_value + Sale.q.surcharge_value,
        # NEW -> marcelo maia september 2013
        table_num=SaleTab.q.table_num,
        tab_num=SaleTab.q.tab_num,
        color=SaleTab.q.color)
    ...
    joins = [
        ...
        LEFTJOINOn(None, SaleItemIpi,
                   SaleItemIpi.q.id == SaleItem.q.ipi_infoID),
        # marcelo september 2013
        INNERJOINOn(None, SaleTab,
                    Sale.q.id == SaleTab.q.saleID),
    ]
...
class SaleReturnedItemsView(Viewable):
    columns = dict(
        id=SaleReturnedItem.q.id,
        quantity=SaleReturnedItem.q.quantity,
        price=SaleReturnedItem.q.price,
        sale_id=Sale.q.id,
        sellable_id=SaleReturnedItem.q.sellableID,
        code=Sellable.q.code,
        description=Sellable.q.description,
        total=SaleItem.q.price * SaleReturnedItem.q.quantity
    )

    joins = [
        INNERJOINOn(None, Sale, Sale.q.id == SaleReturnedItem.q.saleID),
        INNERJOINOn(None, SaleItem, SaleItem.q.id == SaleReturnedItem.q.sale_itemID),
        INNERJOINOn(None, Sellable, Sellable.q.id == SaleReturnedItem.q.sellableID),
    ]
