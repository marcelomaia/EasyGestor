...
class Till(Domain):
    ...
    def needs_closing(self):
        ...
        if self.opening_date.date() + datetime.timedelta(days=1) == datetime.date.today():
            return False
        # Este False permite que o caixa fique aberto indefinidamente, contanto que nao exista ECF
        return False
    ...
    def get_initial_cash_amount(self):
        description = '%%%s%%' % 'Quantia removida do caixa em'
        last_removal_date = self.get_last_removal_date()
        ...
        results = TillEntry.select(
            ...
            clause=AND(OR(TillEntry.q.paymentID == None,
                          Payment.q.methodID == money.id),
                       TillEntry.q.date <= last_removal_date,
                       TillEntry.q.tillID == self.id),
            connection=conn)
        return currency(self.initial_cash_amount +
                        (results.sum('till_entry.value') or 0))

    def get_last_removal_date(self):
        description = '%%%s%%' % 'Quantia removida do caixa em'
        removal = [entry for entry in TillEntry.select(AND(ILIKE(TillEntry.q.description,
                                                                 description),
                                                           TillEntry.q.tillID == self.id)).orderBy('date').reversed()]
        if not removal or len(removal) == 0:
            return self.opening_date
        return removal[0].date

    def get_parcial_balance(self):
        total = self.get_parcial_entries().sum('value') or 0
        return currency(self.get_initial_cash_amount() + total)

    def get_parcial_entries(self):
        last_removal_date = self.get_last_removal_date()
        return TillEntry.select(
            AND(TillEntry.q.tillID == self.id,
                TillEntry.q.date > last_removal_date), connection=self.get_connection())

    def get_parcial_cash_amount(self):
        from stoqlib.domain.payment.method import PaymentMethod
        conn = self.get_connection()
        money = PaymentMethod.get_by_name(conn, 'money')
        last_removal_date = self.get_last_removal_date()
        ...
        return currency(self.get_initial_cash_amount() +
                        (results.sum('till_entry.value') or 0))
