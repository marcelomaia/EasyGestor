...
from stoqlib.lib.statistics import get_query_analisys, get_advertisement
...
def _setup_ui_dialogs():
    ...
    stock_app = environ.find_resource('pixmaps', 'easyloja32x32.png')
...
def send_recurrency_messages():
    conn = api.new_transaction()
    if sysparam(conn).ONLINE_SERVICES:
        from stoqlib.domain.sellable import SaleItemSchedule
        SaleItemSchedule.dispatch_emails(conn)
