...
class Tasks(object):
    ...
    def add_defaults(self):
        items = [
            ...
            (_('Centro de custo'), 'payment_cost_centers', STOQ_PAYMENT_CATEGORY),
        ...
        for label, name, pixbuf in items:
            if name == 'contracts' and not self.is_nfce_active:
                continue
    ...
    def _open_supplier_categories(self):
        trans = api.new_transaction()
        model = self.app.run_dialog(SupplierCategoryDialog, trans)
        api.finish_transaction(trans, model)
        trans.close()
    ...
    def _open_payment_cost_centers(self):
        trans = api.new_transaction()
        model = self.app.run_dialog(PaymentCostCenterDialog, trans)
        api.finish_transaction(trans, model)
        trans.close()
    ...
    def _open_contracts(self):
        self.app.run_dialog(NFeContractListEditor, self.app.conn)
...
class AdminApp(AppWindow):
    ...
    def create_actions(self):
        ...
        actions = [
            ...
            # ("ConfigurePaymentCostCenters", None, _("Payment cost centers..."),
            #  group.get('config_payment_cost_centers')),
    ...
    # def on_ConfigurePaymentCostCenters__activate(self, action):
    #     self.tasks.run_task('payment_cost_centers')
