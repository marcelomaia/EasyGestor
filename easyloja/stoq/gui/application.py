...
from stoqlib.gui.events import StartApplicationEvent, EndApplicationEvent, ShutdownApplicationEvent
from stoqlib.gui.help import show_section
from stoqlib.gui.introspection import introspect_slaves, _foreach_child
...
class AppWindow(GladeDelegate):
    ...
    def __init__(self, app, keyactions=None):
        ...
        StartApplicationEvent.connect(self._on_StartApplicationEvent__activate)
    ...
    def _create_shared_actions(self):
        ...
        actions = [
            ...
            ("BackButton", gtk.STOCK_GO_BACK, _('Voltar'),
             group.get('new_window'),
             _('Volta para o menu inicial')),
        ...
        self.add_tool_menu_actions([
            ("NewToolItem", _("New"), '', gtk.STOCK_DND_MULTIPLE),
    ...
    def _create_shared_ui(self):
        ...
        # self.feedback_bt.connect('clicked', self._on_Feedback__activate)

        # toolbar.insert(self.feedback_bt, 3)
    ...
    def _run_about(self):
        from stoq import version, website, program_name, program_codename, release_date

        info = get_utility(IAppInfo)
        about = gtk.AboutDialog()
        about.set_name(program_name)
        about.set_program_name(program_codename)
        year, month, day = release_date
        version += '\n{}/{}/{}'.format(day, month, year)
        about.set_version(version)
        about.set_website(website)
        about.set_copyright(u'Copyright (C) 2016 EBI - Empresa Brasileira de Informática')
        # about.set_comments('')
    ...
    def _on_Close__activate(self, action):
        EndApplicationEvent.emit(self.current_app.app.name, self.current_app)
    ...
    def _on_StartApplicationEvent__activate(self, appname, app):
        # self.set_advertisement_text()
        pass
    # Edit
