...
from stoqlib.domain.production import ProductionOrder, ProductionService, ProductionItem, ProductionMaterial, \
    ProductionProducedItem, ProductionItemQualityResult
...
class ProductionApp(SearchableAppWindow):
    ...
    def create_actions(self):
        ...
        actions = [
            ...
            # Clone Production
            ('CloneProduction', gtk.STOCK_APPLY, _('Clone production...'),
             group.get('production_edit'),
             _('Edit the selected production')),
    ...
    def on_CloneProduction__activate(self, widget):
        order = self.results.get_selected()
        assert order is not None
        id = order.id
        ...
        services = ProductionService.selectBy(order=id)
        items = ProductionItem.selectBy(order=id)
        materials = ProductionMaterial.selectBy(order=id)
        produced_items = ProductionProducedItem.selectBy(order=id)
        quality_test = ProductionItemQualityResult.select(AND
                                                          (ProductionOrder.q.id ==
                                                           ProductionProducedItem.q.orderID,
                                                           ProductionItemQualityResult.q.produced_itemID ==
                                                           ProductionProducedItem.q.id,
                                                           ProductionOrder.q.id == id))

        branch = api.get_current_branch(self.conn)
        model = ProductionOrder(branch=branch,
                                description=desc,
                                status=ProductionOrder.ORDER_OPENED,
                                connection=self.conn)
        for i in items:
            ProductionItem(order=model,
                           product=i.product,
                           quantity=i.quantity,
                           connection=self.conn)
        for s in services:
            ProductionService(order=model,
                              quantity=s.quantity,
                              service=s.service,
                              connection=self.conn)
        for m in materials:
            ProductionMaterial(order=model,
                               needed=m.needed,
                               consumed=m.consumed,
        ...
        last_serial = ProductionProducedItem.selectBy(connection=self.conn).max('serial_number')

        for index, p in enumerate(produced_items):
            ProductionProducedItem(order=model,
                                   product=p.product,
                                   produced_by=p.produced_by,
                                   serial_number=last_serial+index+1,       # continue from last serial
                                   entered_stock=False,
                                   produced_date=p.produced_date,
                                   test_passed=False,
                                   connection=self.conn)
        for q in quality_test:
            ProductionItemQualityResult(produced_item=q.produced_item,
                                        quality_test=q.quality_test,
