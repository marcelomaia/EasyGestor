...
from stoqlib.domain.events import CreatedOutPaymentEvent
...
from stoqlib.gui.wizards.renegotiationwizard import PayableRenegotiationWizard
...
class PayableApp(SearchableAppWindow):
    ...
    def get_columns(self):
        return [SearchColumn('id', title=_('#'), long_title=_('Payment ID'),
                ...
                SearchColumn('value', title=_('Value'), data_type=currency,
                             width=90),
                SearchColumn('branch_name', title='Na filial', data_type=str,
    ...
    def _show_details(self, payable_view):
        ...
        if retval:
            CreatedOutPaymentEvent.emit(retval)
