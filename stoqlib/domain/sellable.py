...
from stoqlib.lib.email_sender import email_deliver
...
class Sellable(Domain):
    ...
    def generate_barcode(self):
        _lenght = 12
        auto_complete = _lenght-len(self.extract_digits(self.code)) - 3
        barcode = u'789'+self.extract_digits(self.code)
        for i in range(0, auto_complete):
            barcode += (str(random.randint(0, 9)))
        total = 0
        for index, number in enumerate(barcode):
            if index % 2 == 0:
                total += int(number) * 1
            else:
                total += int(number) * 3
        checksum = 10 - total % 10          #http://pt.wikipedia.org/wiki/EAN-13
        if checksum == 10:
            checksum = 0
        barcode += str(checksum)
        if not self.check_barcode_exists(barcode):
            self.barcode = barcode

    def has_recurrency(self, conn):
        return Recurrency.selectOneBy(sellable=self, connection=conn) is not None

    def get_recurrency(self, conn):
        return Recurrency.selectOneBy(sellable=self, connection=conn)
    ...
    @classmethod
    def get_available_sellables_for_increase_query(cls, conn):
        return OR(cls.q.status == cls.STATUS_AVAILABLE,
                  cls.q.status == cls.STATUS_UNAVAILABLE)
...
class Recurrency(Domain):
    TYPE_PERIOD = {u"dias": u"days",
                   u"semanas": u"weeks",
                   u"meses": u"mounths",
                   u"anos": u"years"}

    period = IntCol(default=None)
    type_period = UnicodeCol(default=None)
    subject = UnicodeCol(default=None)
    message = UnicodeCol(default=None)
    sellable = ForeignKey('Sellable')


class SaleItemSchedule(Domain):
    is_sent = BoolCol(default=False)
    shipping_date = DateTimeCol()
    sale_item = ForeignKey('SaleItem')

    @classmethod
    def dispatch_emails(cls, conn):
        schedule_list = cls.select("shipping_date >= now()- interval '1 days' and is_sent=FALSE", connection=conn)
        for sc in schedule_list:
            sc.send_notification(conn)

    def send_notification(self, conn):
        email = self.sale_item.sale.client.person.email
        recurrency = Recurrency.selectOneBy(sellable=self.sale_item.sellable, connection=conn)
        subject = recurrency.subject
        self.is_sent = email_deliver(email, recurrency.message, subject)
        conn.add_modified_object(self)
        conn.commit()
