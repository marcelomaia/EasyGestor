...
class PaymentMethod(Domain):
    ...
    @classmethod
    def get_by_description(cls, conn, description):
        """Returns the Payment method associated by the nmae
        @param description: name of a payment method
        @returns: the payment method class
        @rtype: L{PaymentMethod} instance
        """
        return PaymentMethod.selectOneBy(connection=conn,
    ...
    @classmethod
    def get_method_values(cls, conn):
        return {method.description: method.description
                for method in PaymentMethod.selectBy(is_active=True, connection=conn).orderBy('description')}
