...
class _BasePaymentReport(ObjectListReport):
    ...
    def _setup_table(self):
        total_value = sum([item.value for item in self._payments],
                          Decimal(0))
        paid_value = sum([item.paid_value for item in self._payments if item.paid_value is not None],
                         Decimal(0))
        self.add_summary_by_column(_(u'Value'),
                                   get_formatted_price(total_value))
        self.add_summary_by_column(_(u'Paid'),
                                   get_formatted_price(paid_value))
        p0 = self._payments[0]
        if hasattr(p0, 'real_value'):
            real_value = sum([item.real_value for item in self._payments],
                             Decimal(0))
            self.add_summary_by_column(_('Valor real'),
                                       get_formatted_price(real_value))
