...
from stoqlib.gui.wizards.nfewizard import NFeProductsWizard
...
class PurchaseItemStep(SellableItemStep):
    ...
    def get_columns(self):
        return [
            ...
            Column('ipi_cost', title=_('IPI'), data_type=currency,
                   format_func=get_formatted_cost, width=90),
    ...
    def calc(self, order):
        return sum([p.get_total() for p in PurchaseItem.selectBy(order=order, connection=self.conn)])

    def next_step(self):
        if self.model.consigned:
            return FinishPurchaseStep(self.wizard, self, self.conn, self.model)
        outstanding_value = self.calc(self.model)
        return PurchasePaymentStep(self.wizard, self, self.conn, self.model, outstanding_value)
    ...
    def get_order_xml_item(self, sellable, cost, quantity):
        item = self.model.add_item(sellable, quantity)
        self._set_expected_receival_date(item)
        item.cost = cost
        return item

    def update_sellable_cost(self):
        purshase_item_cost, sellable_cost = self.proxy.model.cost, Decimal(self.proxy.model.sellable.cost)
        sellable = self.proxy.model.sellable

        if purshase_item_cost != sellable_cost:
            if yesno(_("Atualizar preço de custo de %s para R$ %s?" % (sellable.description,
                                                                       purshase_item_cost)),
                     gtk.RESPONSE_YES, _("Yes"), _("No")):
                with api.trans() as trans:
                    sellable = trans.get(sellable)
                    sellable.cost = purshase_item_cost
    ...
    def _add_sellable(self):
        self.update_sellable_cost()
        super(PurchaseItemStep, self)._add_sellable()
    ...
    def on_xml_button__clicked(self, button):
        model = run_dialog(NFeProductsWizard, get_current_toplevel(), self.conn, None, False)
        try:
            for product in model.products:
                quantity = Decimal(product.quantity)
                cost = Decimal(product.price)
                item = self.get_order_xml_item(product.sellable, cost, quantity)
                if item is None:
                    return
            ...
            print "model não foi resgatado, diálogo foi fechado antes disso"
