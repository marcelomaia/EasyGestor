...
from stoqlib.lib.nfce.nfceutils import get_cities_by_name
from stoqlib.lib.translation import stoqlib_gettext
from stoqlib.lib.validators import validate_state
from stoqlib.lib.viacep import ViaCEP
...
class AddressSlave(BaseEditorSlave):
    ...
    def __init__(self, conn, person, model=None, is_main_address=True,
        ...
        self.cep_search.set_tooltip_text('Clique para cadastrar pelo cep')
    ...
    def _update_cep(self):
        cep = ''.join([d for d in self.postal_code.read() if d in '0123456789'])
        vcp = ViaCEP(cep)
        dic = vcp.retorna_json_completo()
        if dic:
            self.district.update(dic.get('bairro'))
            self.street.update(dic.get('logradouro'))
            self.complement.update(dic.get('complemento'))
            self.city.update(dic.get('localidade'))
            self.state.update(dic.get('uf'))
    ...
    def _complete_cities(self):
        ...
        if city and len(city) >= 2:
            ...
            cities = set(c.city_name for c in get_cities_by_name(city))
    ...
    def on_postal_code__activate(self, entry):
        self._update_cep()

    def on_cep_search__clicked(self, arg):
        self._update_cep()
