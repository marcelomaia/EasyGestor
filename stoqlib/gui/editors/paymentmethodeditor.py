...
class PaymentMethodEditor(BaseEditor):
    ...
    def _setup_widgets(self):
        ...
        if manager.is_active('boleto') and self.model.method_name == 'bill':
            self.daily_penalty.set_sensitive(False)
            self.daily_penalty.set_tooltip_text(u'O plugin permite somente 1% de multa dividido ao longo do mês')
            self.model.daily_penalty = 1.0
...
class CardPaymentMethodEditor(PaymentMethodEditor):
    ...
    def _on_edit_buton_clicked(self, button):
        run_dialog(CardProviderSearch, get_current_toplevel(), self.conn)
