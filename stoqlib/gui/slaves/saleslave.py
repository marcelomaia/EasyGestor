...
class SaleListToolbar(GladeSlaveDelegate):
    ...
    def update_buttons(self):
        ...
        can_edit = bool(sale_view and sale_view.status in [Sale.STATUS_QUOTE, Sale.STATUS_ORDERED])
    ...
    def _edit_sale(self, sale_view):
        trans = api.new_transaction()
        sale_view = trans.get(sale_view.sale)
        model = run_dialog(SaleQuoteWizard, self.parent, trans, sale_view)
        retval = api.finish_transaction(trans, model)
        trans.close()
