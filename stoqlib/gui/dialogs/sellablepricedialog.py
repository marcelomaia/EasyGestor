...
class SellableView(Viewable):
    ...
    def set_markup(self, category, markup):
        price = self.price + self.price * markup / 100
        if price <= 0:
            price = Decimal('0.01')
...
class SellablePriceDialog(BaseEditor):
    ...
    def _setup_widgets(self):
        ...
        change_button_appearance(self.apply, gtk.STOCK_APPLY, 'Aplicar')
    ...
    def _get_columns(self):
        ...
        columns = [Column("code", title=_(u"Code"), data_type=str,
                   ...
                   Column("category_description", sorted=True,
                          title=_(u"Category"), data_type=str, width=100),
        ...
        for cat in self.categories:
            self.adj = gtk.Adjustment(upper=MAXINT, step_incr=currency(1))
            columns.append(Column('price_%s' % (cat.id,),
                                  title=cat.get_description(), data_type=currency, width=90,
                                  visible=True, editable=True, spin_adjustment=self.adj))
    ...
    def _after_objectlist__attr_edited(self, objectlist, obj, item, attr):
        # WORKARROUND
        category_id = int(''.join(number for number in attr if number in '0123456789'))
        category = ClientCategory.selectOneBy(id=category_id, connection=self.conn)
        obj.set_price(category, item)
    ...
    def setup_slaves(self):
        ...
        self.slave.listcontainer.list.connect_after('attr-edited', self._after_objectlist__attr_edited)
        # self.slave.listcontainer.list.connect('cell-edited', self._on_objectlist__cell_edited)
