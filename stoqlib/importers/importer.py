...
_available_importers = {
    ...
    'jud_client.csv': 'jud_client.JudClientImporter',
...
class Importer(object):
    ...
    def process(self, trans=None):
        ...
        print 'Importing %s items' % str(n_items)
        print 'ITEMS:%s' % str(n_items)
        t1 = time.time()
        ...
        if not self.dry:
            trans.commit(close=True)
        t2 = time.time()
        print '%s Imported %s entries in %s sec' % (str(datetime.datetime.now()), str(n_items), str(t2 - t1))
        print 'IMPORTED-ITEMS:%s' % str(imported_items)
