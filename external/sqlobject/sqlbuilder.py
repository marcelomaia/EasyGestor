...
class LIKE(SQLExpression):
    ...
    def __sqlrepr__(self, db):
        escape = self.escape
        expr = const.unaccent(self.expr)
        value = const.unaccent(self.string)
        like = "%s %s (%s)" % (sqlrepr(expr, db), self.op, sqlrepr(value, db))
        if escape is None:
            return "(%s)" % like
        else:
...
class UNACCENT(SQLExpression):
    op = "unaccent"

    def __init__(self, string, escape=None):
        self.string = string
        self.escape = escape

    def __sqlrepr__(self, db):
        unaccent = "%s(%s)" % (self.op, sqlrepr(self.string, db))
        return unaccent
